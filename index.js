...
// Initialize Groq (lazy — only if API key is present)
let groq = null;
if (!process.env.GROQ_API_KEY) {
  console.warn('⚠️  GROQ_API_KEY is not set — translation features will be disabled.');
} else {
...
io.on("connection", socket => {
  ...
  socket.on("send-audio", async (audioData) => {
    if (!groq) {
      socket.emit("transcription-error", { error: "Translation unavailable: GROQ_API_KEY not configured." });
      return;
    }
    try {
  ...
  socket.on('whiteboard-draw', ({ roomId, x0, y0, x1, y1, color, size, eraser }) => {
    socket.to(roomId).emit('whiteboard-draw', { x0, y0, x1, y1, color, size, eraser });
  });

  socket.on('whiteboard-clear', ({ roomId }) => {
    const room = rooms.get(roomId);
    if (room) room.whiteboardSnapshot = null;
    socket.to(roomId).emit('whiteboard-clear');
  });

  socket.on('whiteboard-sync-state', ({ roomId, imageData }) => {
    const room = rooms.get(roomId);
    if (room) room.whiteboardSnapshot = imageData;
  });

  socket.on('whiteboard-request-sync', ({ roomId }) => {
    const room = rooms.get(roomId);
    if (room && room.whiteboardSnapshot) {
      socket.emit('whiteboard-sync', { imageData: room.whiteboardSnapshot });
    }
  });
